...
Weekly AI stock picker — Claude Haiku + yfinance
"""
...
def get_market_snapshot(tickers: list[str]) -> str:
    ...
    for t, p in prices.items():
        lines.append(f"  {t}: ${p}" if p else f"  {t}: N/A")
    return "\n".join(lines)
...
def build_prompt(portfolio: dict, prices: dict) -> str:
    ...
    for cat, tickers in CANDIDATE_UNIVERSE.items():
        snap = get_market_snapshot(tickers[:8])
...
TASK: Decide this week's portfolio actions to move toward target allocations.
Rules:
...
{{
  ...
  "weekly_outlook": "<2-3 sentences market view>",
  "cash_after": <float>
}}"""
...
def main():
    ...
    all_tickers = [pos["ticker"] for pos in portfolio["positions"]]
    ...
    if sp500_price and portfolio["history"]:
        first = portfolio["history"][0]
        first_sp500 = first.get("sp500_price") or sp500_price
